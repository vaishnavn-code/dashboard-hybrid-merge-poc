export function mapPortfolioMix(rawData) {
  ...
  */
  ...
  const portfolioTrendData = Object.entries(portfolioTrendRaw)
    .map(([month, products]) => {
      ...
      const commercialPaper =
        products?.["Commercial Paper"] || products?.["Commerical Paper"] || {};
      ...
      return {
        ...
        commercialPaperWtIntAmtEir: Number(
          commercialPaper?.wt_int_amt_eir || 0,
        ),
        ...
        othersWtIntAmtCouponYield: Number(others?.wt_int_amt_coupon_yield || 0),
        loansWtIntAmtCouponYield: Number(loans?.wt_int_amt_coupon_yield || 0),
        ...
        othersWtAmtCouponYield: Number(others?.wt_amt_coupon_yield || 0),
        loansWtAmtCouponYield: Number(loans?.wt_amt_coupon_yield || 0),
      };
    })
    .slice(-13);
  ...
  const barLineTrendRaw =
    portfolioCharts?.["Portfolio Bar Line Trend"]?.values || [];

  const barLineTrendData = barLineTrendRaw.map((item) => ({
    name: item.month,
    opening: Number(item.opening || 0),
    closing: Number(item.closing || 0),
    avgEir: Number(item.avgEir || 0),
  }));
